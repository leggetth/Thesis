from nistsp80022tests import block_freq, freq_monobit, runs, longestrunofones, cumulative_sums
...
class Sensitivity_Analysis():
    ...
    def f(self, ga_array):
        ...
        total_p_value = (self.w1 * p_value_list[0]) + (self.w2 * p_value_list[1]) + (self.w3 * p_value_list[2]) +\
                        (self.w4 * p_value_list[3]) + (self.w5 * (p_value_list[4][0] + p_value_list[4][1]))
            ...
            for w3 in arange(0.0, 4/3, 1/3):
                for w4 in arange(0.0, 4/3, 1/3):
                    for w5 in arange(0.0, 4/3, 1/3):
                        sa = Sensitivity_Analysis([block_freq, freq_monobit, runs, longestrunofones, cumulative_sums],
                                                  w1, w2, w3, w4, w5)
                        eval_output = sa.evaluation()
                        if eval_output["eval_value"] > best_eval_value:
                            best_weights = {"w1": w1, "w2": w2, "w3": w3, "w4": w4, "w5": w5}
